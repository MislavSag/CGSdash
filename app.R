...
ui = bslib::page_navbar(
  ...
  bslib::nav_panel("Overview", layout_columns(
    ...
    card(
      ...
      layout_sidebar(sidebar = sidebar(
        checkboxInput("leverage", "Leverage", value = FALSE)
  ...
  bslib::nav_panel("AlphaPicks", module_strategy_ui("alphapicks", alpha_ui_alphapicks, "https://snpmarketdata.blob.core.windows.net/descriptions/AlphaPicks.html")),
  bslib::nav_panel("VIXCombo", module_strategy_ui("vixcombo", alpha_ui_vixcombo, "https://snpmarketdata.blob.core.windows.net/descriptions/VIXComboV2.html")), # alpha_ui_vixcombo
...
server = function(input, output) {
  ...
  o_vixcombo = module_strategy_server(
    "vixcombo",
    FLEX_VIXCOMBO,
    start_date = vixcombo_start,
    strategy_leverage = 1.8,
    end_date = NULL,
    alpha = alpha_server_vixcombo # alpha_server_vixcombo
  )
...
#  )
  output$dt_shared_returns = DT::renderDT({
    leverage_ = if (isTRUE(input$leverage)) NULL else 2.0
    dates = c(o_riskcombo()$date,
              o_alphapicks()$date,
              o_vixcombo()$date,
              o_pra()$date,
              o_minmax()$date,
    ...
    dt_ = cbind(
      get_portfolio_stats_from_strategy(o_riskcombo()$strategy, o_riskcombo()$date, if (isTRUE(input$leverage)) NULL else 2.66), # leverage_
      get_portfolio_stats_from_strategy(o_alphapicks()$strategy, o_alphapicks()$date, if (isTRUE(input$leverage)) NULL else 2.0),
      get_portfolio_stats_from_strategy(o_vixcombo()$strategy, o_vixcombo()$date, if (isTRUE(input$leverage)) NULL else 1.8),
      get_portfolio_stats_from_strategy(o_pra()$strategy, o_pra()$date, if (isTRUE(input$leverage)) NULL else 2.66),
      get_portfolio_stats_from_strategy(o_minmax()$strategy, o_minmax()$date, if (isTRUE(input$leverage)) NULL else 2.66),
      get_portfolio_stats_from_strategy(o_exuber()$strategy, o_exuber()$date, if (isTRUE(input$leverage)) NULL else 2.66)
    ...
    dt_portfolio(dt_, "strategy_performance", dates) |>
      ...
      formatStyle(columns = 12:13, backgroundColor = "#ffe4e1") #|>
  ...
  output$ui_strategies_in_positions <- renderUI({
    ...
    has_vixcombo = o_vixcombo()$portfolio_has_asset
    ...
    tagList(
      ...
      tags$div(
        lapply(names(strategy_status), function(strat) {
          ...
          if (in_position %notin% c("FALSE", "TLT", "VXX")) {
            # GREEN BADGE
            tags$span(
